...
echo "docker run --restart=always -e NUM_INDEXER_WORKERS=0 -e TXPOOL_VERSION=0 -e GETH_PROTOCOL=https -e GETH_NAME=eth-fullnode.incognito.org -e GETH_PORT="" -e NAME="inc-nodes" -e TESTNET=false -p 9334:9334 -p 9335:9335 -p 9336:9336 -e BOOTNODE_IP=mainnet-bootnode.incognito.org:9330 -v ./data:/data -e MININGKEY=$miningkeys_str -e PUBLIC_IP=$public_ip  -e NODE_PORT=9334 -e WS_PORT=9336 -e RPC_PORT=9335 -d --name inc-nodes incognitochain/incognito-mainnet:$chain_tag bash /run_incognito.sh"
